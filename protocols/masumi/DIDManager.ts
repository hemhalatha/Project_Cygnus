...
export class DIDManager {
  ...
  private didCache: Map<DID, DIDDocument> = new Map();
   ...
   */
  async updateDID(did: DID, updates: DIDUpdate): Promise<DIDDocument> {
    ...
    if (updates.removeVerificationMethod) {
      updatedDoc.verificationMethod = updatedDoc.verificationMethod.filter(
        (vm) => !updates.removeVerificationMethod!.includes(vm.id)
      );
    }
    ...
    if (updates.removeService) {
      updatedDoc.service = updatedDoc.service.filter(
        (s) => !updates.removeService!.includes(s.id)
      );
    }
   ...
   * Deactivate DID
   */
  async deactivateDID(did: DID): Promise<void> {
    const resolution = await this.resolveDID(did);
    if (!resolution.didDocument) {
      throw new Error(`DID not found: ${did}`);
    }
   ...
   */
  private generateDID(publicKey: string): DID {
   ...
   * Store DID document
   */
  private async storeDIDDocument(didDocument: DIDDocument): Promise<void> {
