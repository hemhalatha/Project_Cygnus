...
export class RiskAssessor {
  ...
  private transactionHistory: Map<DID, TransactionHistoryEntry[]> = new Map();
   ...
   */
  private assessCreditRisk(reputation: ReputationScore): RiskFactor {
   ...
   */
  private assessHistoryRisk(counterpartyDID: DID): RiskFactor {
    ...
    const recentTxs = history.filter(
      (tx) => Date.now() - tx.timestamp < 30 * 24 * 60 * 60 * 1000 // 30 days
    );
   ...
   */
  private assessTransactionSizeRisk(amount: number): RiskFactor {
   ...
   */
  private assessCapabilityRisk(metadata: AgentMetadata): RiskFactor {
